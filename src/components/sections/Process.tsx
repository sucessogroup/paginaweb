...
export const Process = () => {
  return (
    <section id="proceso" className="py-24 md:py-40 bg-[#19373E]">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-24 space-y-6">
          <h2 className="text-[10px] uppercase tracking-[0.5em] text-brand-canary font-bold">Nuestro Método</h2>
          <h3 className="text-4xl md:text-6xl font-headline font-bold text-white">Orden que genera <span className="italic font-light text-brand-canary">confianza.</span></h3>
          <div className="w-20 h-[1px] bg-brand-canary mx-auto mt-8" />
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12">
          {steps.map((step, idx) => (
            <div key={idx} className="group relative p-10 bg-white/5 rounded-[2.5rem] border border-white/10 hover:bg-white/10 transition-all duration-500">
              <div className="mb-8 w-16 h-16 rounded-2xl bg-brand-canary/10 flex items-center justify-center text-brand-canary group-hover:bg-brand-canary group-hover:text-brand-darkGray transition-all duration-500">
                <step.icon size={32} />
              </div>
